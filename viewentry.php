...
if(isset($_POST['submit'])) {
	...
	if(isset($session->isLoggedIn() == true) {
	...
	$comment = Comment::find("SELECT * FROM comments WHERE blog_id = :validId ORDER BY date DESC", ['validId' => $validId]);
	...
	if(count($comments) == 0) {
		...
		foreach($comments as $comment){
			...
			"</p>\n";
			echo "<p class='commentody'>" . $comment->getComment() . 
			"</p>\n";
		}
	}
	echo "</div>\n";
